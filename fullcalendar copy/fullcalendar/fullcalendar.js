	...
	function changeView(newViewName) {
		if (!currentView || newViewName != currentView.name) {
			ignoreWindowResize++; // because setMinHeight might change the height before render (and subsequently setSize) is reached
			  if ((newViewName=='agendaWeek') || (newViewName=='month')) content.css('font-size','11px');
			  else content.css('font-size','18px');
...
function AgendaView(element, calendar, viewName) {
	...
	function renderAgenda(c) {
		 ...
		 $('*').undelegate(".fc-axis", "click").delegate(".fc-axis", "click", function(event) 
			{
			event.stopPropagation();										    
			$('.fc-day-content').slideToggle('slow');
			});
		...
		if (!head) { // first time rendering, build from scratch
			...
			daySegmentContainer = $("<div id='myallday' style='position:absolute;z-index:8;top:0;left:0'/>").appendTo(head);
