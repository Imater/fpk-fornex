...
<tr><td colspan=2 align=center style="font-size:18;height:10">PHP Obfuscator by DX $ver</td></tr>
...
<br><br><a href='http://kaimi.ru/' target='_blank'>&copy; dx</a>
...
Количество проходов: <select name='pnum'><option value='1' selected>1</option><option value='2'>2</option><option value='3'>3</option></select><hr>
...
if($pnum<1 || $pnum>3)
{
  errpr("Недопустимое число проходов (разрешено от 1 до 3).");
}
  ...
  for($i=0;$i<$c;$i++)
  {
    ...
    if(is_array($token))
    {
      ...
      if($ot)
      {
        ...
        if($obf_num && $tn==T_LNUMBER && !$isfunc && $prev!=T_EXIT)
        {
          eval('$tmp='.$ts.';');
          $ts='0';
        ...
        if($obffuncs && $isfunc && $tn==T_STRING && strtolower($ts)!='true' && strtolower($ts)!='false')
        {
